...
{% include card.html type='primary' content='注意本文中的 $$\alpha_t$$ 和 $$\beta_t$$ 的定义和 DDPM 原文中的不一样.' %}

反复迭代 \eqref{eq:add_noise} 式, 可以得到

$$
\begin{align} \nonumber
    \bm{x}_t &= \alpha_t\bm{x}_{t-1} + \beta_t\bm{e}_t \\ \nonumber
              &= \alpha_t(\alpha_{t-1}\bm{x}_{t-2} + \beta_{t-1}\bm{e}_{t-1}) + \beta_t\bm{e}_t \\ \nonumber
              &= \alpha_t\alpha_{t-1}\bm{x}_{t-2} + \alpha_t\beta_{t-1}\bm{e}_{t-1} + \beta_t\bm{e}_t \\ \nonumber
              &= \dots \\ \label{eq:xt_x0}
              &= (\alpha_t\dots\alpha_1)\bm{x}_0 + \underbrace{(\alpha_t\dots\alpha_2)\beta_1\bm{e}_1 + (\alpha_t\dots\alpha_3)\beta_2\bm{e}_2 + (\alpha_t)\beta_{t-1}\bm{e}_{t-1} + \beta_t\bm{e}_t}_{\text{记为} \bm{\gamma}_t}
\end{align}
$$

上式中, $$\bm{\gamma}_t$$ 是相互独立的 $$t$$ 个正态分布的噪声 $$(\alpha_t\dots\alpha_{i+1})\beta_i\bm{e}_i\sim\mathcal{N}(\bm{0},(\alpha_t\dots\alpha_{i+1})^2\beta_i^2\bm{I}),$$ $$i=1\dots t$$ 之和 (注意 $$i=t$$ 时没有 $$\alpha$$ 项.). 根据正态分布的叠加性, 可知 $$\bm{\gamma}_t$$ 的均值为 $$\bm{0}$$, 方差为 $$\sigma^2\bm{I}$$. 根据前面的定义 $$\alpha_t^2 + \beta_t^2=1$$, 我们有
...
$$ \label{eq:xt_x0_2}
\bm{x}_t = \underbrace{(\alpha_t\dots\alpha_1)}_{\text{记为} \bar{\alpha}_t}\bm{x}_0 + \underbrace{\sqrt{1 - (\alpha_t\cdots\alpha_1)^2}}_{\text{记为} \bar{\beta_t}}\bar{\bm{e}}_t, \quad, \bar{\bm{e}}_t\sim\mathcal{N}(\bm{0}, \bm{I}).
$$

那么, 当我们选择合适的 $$\alpha_t$$ 以及足够大的 $$T$$, 最终 $$\bar{\alpha}_t$$ 就会趋于 $$0$$, 这意味着图像信息最终消失, 全部转化为高斯噪声. 

{% include card.html type='info' content='注意 \eqref{eq:xt_x0_2} 式除了表示图像生成噪声的过程, 同时也给出了任意 $$t$$ 步后的加噪图像用原始图像 $$\bm{x}_0$$ 表达的形式, 这对于后续模型训练很有帮助.' %}


## 损失函数计算

那么从噪声生成图像时, 如何训练 $$\bm{x}_{t-1}=\bm{\mu}(\bm{x}_t)$$ 呢? 最直接的方法就是 $$L_2$$ 损失:

$$
\Vert \bm{x}_{t-1} - \bm{\mu}(\bm{x}_t) \Vert_2^2.
$$

这时可以直接用神经网络建模 $$\bm{\mu}(\cdot)$$, 考虑到 $$\bm{x}_{t-1}$$ 和 $$\bm{x}_t$$ 在加噪过程中是有关系的, 所以我们不妨展开一下. 首先式 \eqref{eq:add_noise} 可以写成 $$\bm{x}_{t-1} = \frac1{\alpha_t}(\bm{x}_t - \beta_t\bm{e}_t)$$, 这启发我们可以把 $$\bm{\mu}(\cdot)$$ 写为

$$ \label{eq:inference0}
\bm{\mu}(\bm{x}_t)=\frac1{\alpha_t}(\bm{x}_t - \beta_t\bm{\epsilon}_{\bm{\theta}}(\bm{x}_t, t)),
$$

其中 $$\bm{\epsilon}_{\bm{\theta}}(\bm{x}_t, t)$$ 是我们用神经网络建模的对象, $$\theta$$ 表示模型参数, $$t$$ 为第几步, 模型从噪声生成图像, 因而依赖于 $$\bm{x}$$. 这里引入 $$t$$ 是为每一步生成过程使用不同的生成模型, 通过共享模型参数, 可以把 $$t$$ 作为参数输入模型. 我们得到

$$
\Vert \frac1{\alpha_t}(\bm{x}_t - \beta_t\bm{e}_t) - \frac1{\alpha_t}(\bm{x}_t - \beta_t\bm{\epsilon}_{\theta}(\bm{x}_t, t)) \Vert_2^2 = \frac{\beta_t^2}{\alpha_t^2}\Vert \bm{e}_t - \bm{\epsilon}(\bm{x}_t, t) \Vert_2^2.
$$

因为这是损失函数, 我们忽略前面的系数 $$\frac{\beta_t^2}{\alpha_t^2}$$. 注意到现在损失函数中包含三个变量, 其中 $$\bm{e}_t$$ 是服从标准正态分布的变量, $$\bm{\epsilon}$$ 是神经网络模型, 只有 $$\bm{x}_t$$ 是未知的, 需要通过 $$t$$ 步的加噪采样得到, 这样显得太过复杂. 回忆我们在上一节讨论加噪时, \eqref{eq:xt_x0_2} 式给出可以使用初始图像 $$\bm{x}_0$$ 和服从正态分布的噪声 $$\bar{e}_t$$ 来表示 $$\bm{x}_t$$, 因此我们可以把 \eqref{eq:xt_x0_2} 式代入损失函数得到

$$ \label{eq:loss_ori0}
\Vert \bm{e}_t - \bm{\epsilon}(\bar{\alpha}_t\bm{x}_0 + \bar\beta_t \bar{\bm{e}}_t, t) \Vert_2^2.
$$

上式存在一个问题, 由于 $$\bm{e}_t$$ 和 $$\bar{\bm{e}}_t$$ 不是相互独立的, 采样时会相互影响. 所以我们后退一步使用 $$\bm{x}_{t-1}$$ 对应的 \eqref{eq:xt_x0_2} 式来避免上面的问题. 我们有

$$
\begin{align} \nonumber
    \bm{x}_t &= \alpha_t\bm{x}_{t-1} + \beta_t\bm{e}_t \\ \nonumber
              &= \alpha_t(\bar\alpha_{t-1}\bm{x}_0 + \bar\beta_{t-1}\bar{\bm{e}}_{t-1}) + \beta_t\bm{e}_t \\
              &= \bar\alpha_t\bm{x}_0 + \alpha_t\bar\beta_{t-1}\bar{\bm{e}}_{t-1} + \beta_t\bm{e}_t.
\end{align}
$$
...
$$ \label{eq:loss_ori1}
\Vert \bm{e}_t - \bm{\epsilon}(\bar\alpha_t\bm{x}_0 + \alpha_t\bar\beta_{t-1}\bar{\bm{e}}_{t-1} + \beta_t\bm{e}_t, t) \Vert_2^2.
$$

对比 \eqref{eq:loss_ori0} 式和 \eqref{eq:loss_ori1} 式, 本质就就是把 $$\bar\beta_t\bar{\bm{e}}_t$$ 替换为了 $$\alpha_t\bar\beta_{t-1}\bar{\bm{e}}_{t-1} + \beta_t\bm{e}_t$$, 但是这样就把一个正态分布采样变成了两个, 这可能会增大模型训练的难度. 而使用 \eqref{eq:loss_ori0} 式又会导致两个正态分布产生依赖无法采样, 那么有没有办法在 \eqref{eq:loss_ori0} 式中把 $$\bm{e}_t$$ 替换成不和 $$\bar{\bm{e}}_t$$ 直接依赖的正态分布呢? 
