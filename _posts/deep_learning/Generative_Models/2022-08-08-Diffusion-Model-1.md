...
假设输入的图像为 $$\pmb{x}$$, 最终变成的噪声图像是 $$\pmb{z}$$, 通过 $$T$$ 步把图像变成噪声, 那么就有:

$$
\pmb{x} = \pmb{x}_0\rightarrow \pmb{x}_1\rightarrow \pmb{x}_2\rightarrow \cdots \pmb{x}_{T-1}\rightarrow \pmb{x}_T = \pmb{z}
$$

这样我们就把复杂的图像到噪声的变换过程拆成了一个个的小的步骤, 每个步骤 $$\pmb{x}_{t-1} \rightarrow \pmb{x}_t$$ 我们只需要加一点点噪声. 那么这个过程反过来就变成了 $$\pmb{x}_t\rightarrow \pmb{x}_{t-1}$$ 也就变成了只需要去一点点噪声. 这时候我们只需要学习一个可以去掉一点噪声的模型 $$\pmb{x}_{t-1} = \pmb\mu(\pmb{x}_t)$$, 然后反复地进行去噪 $$\pmb{x}_{T-1}=\pmb\mu(\pmb{x}_{T})$$, $$\pmb{x}_{T-2}=\pmb\mu(\pmb{x}_{T-1}), \dots$$ 就可以获得一幅具有语义信息的图像 $$\pmb{x}_0$$ 了. 

如何建模去噪模型 $$\pmb\mu(\cdot)$$ 呢? 我们首先考虑怎么加噪. 具体来说, 我们可以把加噪的过程看作图像和噪声 MixUp 的过程, 
...
$$ \label{eq:xt_x0_2}
\pmb{x}_t = \underbrace{(\alpha_t\dots\alpha_1)}_{\text{记为} \bar{\alpha}_t}\pmb{x}_0 + \underbrace{\sqrt{1 - (\alpha_t\cdots\alpha_1)^2}}_{\text{记为} \bar{\beta_t}}\bar{\pmb{e}}_t, \quad, \bar{\pmb{e}}_t\sim\mathcal{N}(\pmb{0}, \pmb{I}).
$$
